...
# Shapes returned to the client. Highlight is a span into the raw verse text
# ({"start": int, "end": int, "kind": "first" | "last" | "name"}); Payload is
# any JSON-serializable response object -- both are dicts of mixed value
# types, which mypy can't express more precisely without a TypedDict per
# shape, so `Any` stands in for the value type on Payload specifically.
...
def find_pairs(corpus: Corpus, first_name: Name, second_name: Name, gap: int) -> list[Payload]:
