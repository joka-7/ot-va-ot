...
to wake: the page paints immediately, and its own `/api/health` call starts
Render waking right then — so the wake runs while the visitor is reading the
page and typing a name. Installed as a PWA it is better still, because the
service worker's cached shell opens straight from the icon.
...
"waking the server" notice stays up and the search completes on its own.
